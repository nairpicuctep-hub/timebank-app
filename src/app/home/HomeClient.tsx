        ...
        <div className="grad-card rise p-5">
          ...
          <div className="flex justify-between items-end mt-3 relative">
            <div>
              ...
              <div style={{ fontSize: 12, opacity: 0.85, marginTop: 4 }}>Available to spend</div>
            </div>
            <div className="text-right">
              <div className="font-display font-bold" style={{ fontSize: 22 }}>{earned % 1 === 0 ? earned : earned.toFixed(1)}</div>
              <div style={{ fontSize: 12, opacity: 0.85 }}>Earned</div>
            </div>
          </div>
          {escrow > 0 && (
            <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-pill relative"
              ...
              ⏳ {escrow % 1 === 0 ? escrow : escrow.toFixed(1)} TC held in escrow
            </div>
          )}
          <div className="flex justify-between items-center mt-3 relative" style={{ fontSize: 12 }}>
            <span style={{ opacity: 0.85 }}>Spent: {spent % 1 === 0 ? spent : spent.toFixed(1)} TC</span>
        ...
        <div className="glass rise-1 p-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="font-display font-semibold text-[19px] grad-text">{LEVEL_NAMES[level]}</div>
            </div>
          ...
          <div className="mt-4">
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <b className="font-mono text-ink">{xp} / {level < 7 ? nextXP : 'MAX'} XP</b>
            </div>
              ...
              <Link href={`/session/${nextSession.id}`}>
                <div className="rise-2 p-5 relative" style={{ borderRadius: 'var(--r-card)', overflow: 'hidden',
                  ...
                  <h3 className="font-display font-semibold text-[21px] mt-2.5 mb-2.5 relative">
                    {nextSession.skill?.icon} {nextSession.skill?.name || 'Session'}
                  </h3>
                  <div className="flex items-center gap-2 mb-3 relative">
                    ...
                    <span className="text-[13px] font-medium">
                      {nextSession.teacher_id === uid
                        ? `Teaching ${nextSession.learner?.full_name || 'learner'}`
                        : `with ${nextSession.teacher?.full_name || 'teacher'}`}
                    </span>
                  </div>
                  ...
                  <div className="glass overflow-hidden">
                    ...
                    <div className="p-3.5">
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {t.skills.slice(0, 4).map((s: any, j: number) => (
